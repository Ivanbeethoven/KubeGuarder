...
@Component
@Slf4j
@ServerEndpoint("/api/ws/sensitive/{sid}")
public class SensitiveWebSocketServer {
    //静态变量，用来记录当前在线连接数。应该把它设计成线程安全的。
    private static AtomicInteger onlineCount = new AtomicInteger(0);
    //concurrent包的线程安全Set，用来存放每个客户端对应的WebSocket对象。
    private static CopyOnWriteArraySet<SensitiveWebSocketServer> webSocketSet = new CopyOnWriteArraySet<>();
    ...
    public void onMessage(String message, Session session) {
        ...
        for (SensitiveWebSocketServer item : webSocketSet) {
            sids.add(item.sid);
        }
        try {
            //返回原始内容
     ...
     * @return
     */
    public static CopyOnWriteArraySet<SensitiveWebSocketServer> getWebSocketSet() {
        return webSocketSet;
    }
